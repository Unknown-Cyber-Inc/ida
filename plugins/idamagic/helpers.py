...
# Default minimum length of string to return for strings()
MIN_STRING_LENGTH = 3
...
def gen_random_idb_filename(length=10):
    ...
    extension = ".idb" if arch == "32-bit" else ".i64"

    return f"{rand_filename}{extension}"
...
def get_idb_byte_list() -> list:
    ...
    bytelist = list()
    # Get first segment (seg = segment_t * or None)
    seg = ida_segment.get_first_seg()
    # Iterate over available segments
    while seg is not None:
        # Iterate over all ea's in the segment
        for ea in range(seg.start_ea, seg.end_ea):
            # For each ea, get the flags
